...
export default function AgendaScreen() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      // token ausente ou expirado — manda de volta pro login em vez de deixar a promise sem tratamento
  ...
  useEffect(() => {
    AsyncStorage.getItem("professional").then((raw) => {
      if (!raw) return;
      const professional = JSON.parse(raw);
      setProfessionalName(professional.name);
      setProfessionalSlug(professional.slug);
    });
  }, []);
  ...
  const shareLink = () => {
    if (!professionalSlug) {
      // ainda nao carregou os dados do profissional — evita compartilhar um link quebrado
      return;
    }
    Share.share({ message: `Agende seu horario comigo: ${PUBLIC_BOOKING_URL}/${professionalSlug}` });
  };
